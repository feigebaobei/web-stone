...
function f<T, U extends number>(a: T, b: U) {}
class A<T> {
  public k: T[] = []
  a(p: T) {}
}
...
| undefined || 也叫 void || | |
| void|||| | |
| enums|| 枚举。列出具体的可选的值。它是真实的对象。 |ts@5.8时出现了erasableSyntaxOnly,使enums可被擦除。 | | |
| bigint|||| | |
| symbol|||| | |
...
| Function  ||它与`() => {}`不同。| ts中数据类型是Funtion.js中的数据类型是function | | |
| ReadonlyArray   || 使数组只读|| | |
...
```ts
let a: string = 'a'
type A = `start${string}`// 指定字符串开头的类型。指定结尾，同理。 // 模板字符类型
...
|          | is                      | as                       |
| -------- | ----------------------- | ------------------------ |
| 示例     | `v is Type`             | `v as Type`              |
| 含义     | 判断 v 是否是 Type 类型 | 告诉 ts,v 一定 Type 类型 |
| 运行时间 | 运行时                  | 编译时                   |
| 推荐     | 推荐                    | 不推荐                   |
| 返回值   | boolean                 | 不返回                   |
...
||`Awaited<PromiseT>`|解包Promise后的数据类型。用于then方法。|`type B = Awaited<Promise<Promise<number>>> // type B = number`||
||`Partial<ObjT>`|T类型的所有字段设置为可选字段|||
||`Required<ObjT>`|T类型的所有字段设置为必填字段|||
||`Readonly<ObjT>`|T类型的所有字段设置为只读字段|||
||`Record<Keys, T>`|设置一个对象类型的key类型为Keys，值的类型为T，返回这个对象|`type C = 'ca' \| 'cb'; let a: Record(C, T); => a: {ca: T, cb: T}`||
||`Pick<ObjT, Keys>`|从T类型中提取Keys的类型|`type T = Pick<A, 'a'\|'b'>; => T: {a: xx, b: yy}`|与`T[k]`同效|
||`Omit<ObjT, Keys>`|从T类型中删除Keys的类型|||
...
||`Parameters<FT>`|返回FT类型的参数，返回值是tuple类型。FT类型是方法的类型|||
||`ConstructorParameters<T>`|返回构造方法的参数。返回值是tuple类型。|||
||`ReturnType<FT>`|返回FT类型的返回值的类型|||
...
||`Uppercase<T>`|返回大写|||
||`Lowercase<T>`|返回小写|||
||`Capitalize<T>`|返回首字母大写|||
||`Uncapitalize<T>`|返回首字母小写|||
<!-- prettier-ignore-end -->

```ts
// ts操作类型的方法中没有valueOf。现在自己写一个。
type ValueOf<T> = T[keyof T] // 工具类方法都是大写开头
// 解决继承时，重写方法。
...
// 取数组中的类型
type itemType = typeof arrayVar[number]
...
声明文件就是`*.d.ts`文件。
编译 ts 文件的工作可以生成此文件。比如 tsc 就能生成此文件。
感觉声明文件是给 vscode 使用的。
...
1. `*.d.ts`是声明文件
2. 声明 api/代码结构等。一般用于：在引用了外部 js 代码时需要用声明文件描述 api 等。一般由代码生成，不是手动编写的。
3. 编写好`*.d.ts`后发布到`@types orgnization`
4. 使用`npm i -s @type/xxx`安装。
5. 在`*.js`的同级编写`*.d.ts`。
6. 使用`/// <reference types="..." />`引入其他声明。
7. 有此文件后可在编辑器中进行溯源。
8. 写`*.js + *.d.ts`还不如写`*.ts`.

### 声明文件的作用 (d.ts)

- 提供类型信息，让 TypeScript 知道 JavaScript 代码的结构和类型。
- 在开发时提供代码智能提示和静态类型检查，减少错误。
- 不改变原 JavaScript 代码，只做类型描述。
...
|     | Object    | object     | {}                |
| --- | --------- | --------------------------- | ----------------- |
|     | Object 类 | 网友说无”原始值“（好像是 Object 的原型链上的值），非原始类型（undefined, null, boolean, number, bigint, string, symbol） | 专指没有属性的 object |
...
```ts
import { f, type T } from './one.ts'
import type { T } from './one.ts'
export type { C }

import type T, { G } from './one.ts'
import type { default as T, G } from './one.ts'
```
...
- 脏数据无法在处理。在开发时使用严格的类型判断后执行相应逻辑。在生产时遇到脏数据使运行出错。ts 是静态类型检测。可以在编辑器中要求代码按照类型编写。ts 非运行时类型检测，无法阻止运行时数据流与编写代码时的数据类型不一致的问题发生。当文件是`*.ts`时有类型检测能力，当被`tsc`可其他的转换工具转换为`*.js`时就不具备类型检测能力了。当前的 js 运行环境都不支持直接运行 ts。
