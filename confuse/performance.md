...
- fcp/lcp 影响内容呈现给用户的体验。对页面跳出率影响最大。
- fid/inp/tti，影响用户与网页交互的体验。对功能置换率和网页留存率影响较大。

### FP(First Paint)首次绘制 或者 FCP(First Contentful Paint) 首次内容绘制
...
首次输入延迟，衡量从用户首次与您的网站互动（点击链接、点按按钮或使用由 JavaScript 提供支持的自定义控件）到浏览器实际能够响应该互动的时间
...
累计布局偏移量。会触发重绘、回流。
衡量从页面开始加载到其生命周期状态更改为隐藏之间发生的所有意外布局偏移的累计得分
