...
没学会
当数组长度 n = fib(k) - 1，则 m = fib(k - 1) - 1，分为二个数组。长度：（fib(k-1)-1, fib(k-2)-1）
...
```js
link.find(e, n, p) // 在link中p下标往前n个元素的范围内查找元素e.
link.find(e, p, n) // 在link中p下标往后n个元素的范围内查找元素e.
```
