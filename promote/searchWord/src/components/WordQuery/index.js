...
export default function WordQuery() {
  ...
  let oldSetBox = JSON.parse(
    window.localStorage.getItem('setBox') || JSON.stringify(defaultSetBox)
  )
  ...
  return (
    <main className={styles.appRp}>
      <div className={styles.searchBox}>
        <Search
          defaultValue={parseUrlQS().q || ''}
          ref={searchRef}
          size="large"
          allowClear
          placeholder="input search text for test"
        ...
        {wordList.map((wordItem, index) => {
          return (
            <Fragment key={index}>
              ...
              {isEq(wordItem.entry, wordItem.query) && (
                <p>
                  {wordItem.ukphone && (
                    <span style={{ marginRight: '8px' }}>
                      <span>英</span> <span>{wordItem.ukphone}</span>
                    </span>
                  )}
                  {wordItem.usphone && (
                    <span>
                      <span>美</span> <span>{wordItem.usphone}</span>
                    </span>
                  )}
                </p>
              )}
