...
基于现有前端组件以微卡片形式得到若干功能模块，再支持安逸花相关业务

## 目标

- 每个功能模块以微卡片形式存在，足够独立。可以多次复用。
...
| -        |                                      | 基座支持有限功能             | 创建一个盒子。用户可以在盒子中放入插件。盒子中有什么插件，在项目中就能使用什么功能。     |

# 概念介绍

|                      |                                                                          |     |
| -------------------- | ------------------------------------------------------------------------ | --- |
| 微卡片（简称：卡片） | 本质是组件。内定写技术逻辑，不写业务逻辑。由基础级组件或物料级组件组成。 |     |
| 基座                 | 连接起当前项目中所有微卡片。处理微卡片之间通信。提供若干 api。           |     |
| 模块                 | 可独立完成一块功能的代码。                                               |     |
...
| 粒度     | 小。可以做很小，也可以做很大。薄，基于组件简单封装。 | 大。只做很大。厚，至少有应用级厚度。 |     |
| 可组合性 | 可组合、可嵌套。                                     | 可组合、可嵌套。                     |     |
| 扩展性   | 支持接入到老项目开发。                               | 不支持                               |     |
| 生态系统 | 无社区。必须要有基座。                               | 有公开社区。有插件。                 |     |
| 易用性   | 不需要提前注册。与使用组件基本无异。                 | 需要提前注册。                       |     |
...
|-|-|-|
|来源|来自业务|来自业务|
|体积|小。把多个卡片共用部分移到基座中。|大。可以使用共用 util 包达到相同效果。|
...
沿用组件库的思路。每个卡片都有 4 种 api(props/slots/events/methods).
...
unRegister(id: S) => void // 注册id与组件实例的映射关系
getCard(id: S) => ComponentInternalInstance | undefined // 通过id获取组件实例
getExpose(id: S) => ComponentInternalInstance | undefined // 通过id获取组件实例暴露的对象
registerPlugin(pluginName: S, plugin: A) => void // 注册插件
unRegisterPlugin(pluginName: S) => void // 注销插件
getPlugins(pluginName: S) => A // 取得插件
setProps(id: S, p: Oa) => void // 设置该id对应的组件的props
call<T>(id: S, fn: S) => ReturnType<T> // 调用该id对应的组件实例的指定方法
```
...
let fn = () => {
  console.log('fn')
  pedestal.getPlugins('pedestalPluginName').fn()
}
defineExpose({ fn })
onMounted(() => {
...
```
import pedestal from 'pedestal'
import pedestalPluginName from 'pedestal-plugin-name'
...
一个简单卡片可实现一个基础级功能。多个简单卡片组件后实现一个模块级功能。
支持富代码，不支持低代码。
可以在 vue3 项目中使用微卡片。
微卡片内支持技术逻辑，不支持业务逻辑。
卡片市场支持公开微卡片。
...
- 编写一人示例，展示基础使用方法。包含：定义卡片、使用卡片、给基座注册插件、调用插件。
- 不验收卡片市场、模块市场、基座插件。
